...
class AnuncioController
{
    ...
    public function actualizaAnuncio()
    {
        ...
        if (!empty($data->titulo) and !empty($data->descripcion) and !empty($data->precio) and !empty($data->img_url) and !empty($data->id)){
            $anuncio=new Anuncio($data->id, $data->titulo, $data->descripcion, $data->precio, $data->img_url, null, $usuario[0]['id']);
            $resultado=$anuncio->validaAnuncio();
            if (isset($resultado['error'])){
                ResponseHttp::statusMessage(503, $anuncio['error']);
                exit();
            ...
            if ($anuncioRepository->actualizaAnuncio($anuncio)) {
                ResponseHttp::statusMessage(202, "Anuncio actualizado");
            }else{
                ResponseHttp::statusMessage(503, "No se ha podido actualizar el anuncio");
            }
        }else{
            ResponseHttp::statusMessage(503, "Faltan datos");
            exit();
    ...
    public function listaAnuncios()
    {
        ...
        if (!$anuncios){
            ResponseHttp::statusMessage(503, "No se ha podido obtener la lista de anuncios");
            exit();
        }
        ResponseHttp::statusMessage(200, json_encode($anuncios));
